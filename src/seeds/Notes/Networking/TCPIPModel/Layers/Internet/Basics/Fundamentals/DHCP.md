...
<div class="centeredBullet">
  <ul class="diamondBullets fullWidthBullet">
    <li>Useful for printers, servers, and other networking devices that need to be accessible to clients on the network.</li>
    <li>If hosts normally access a server at a particular IPv4 address, it would not be good if that address changed, static assignment prevents that.</li>
  </ul>
</div>
...
Static addresses also come with some drawbacks:

<div class="centeredBullet">
  <ul class="diamondBullets fullWidthBullet">
    <li>When IPv4 addresses are entered statically, the host only performs basic error checks on the address, so errors are more likely to occur.</li>
    <li>It is important to maintain an accurate list of which IPv4 addresses are assigned to which devices, since these are permanent addresses that are not normally reused.</li>
  </ul>
</div>
...
If the host is powered down or taken off the network, the address is returned to the pool for reuse. This is especially helpful with mobile users that come and go on a network.
...
<div class="centeredBullet">
  <ul class="diamondBullets fullWidthBullet">
    <li><span class="emphasis">IPv4 addresses</span></li>
    <li><span class="emphasis">subnet masks</span></li>
    <li><span class="emphasis">default gateways</span></li>
    <li><span class="emphasis">other configuration information</span></li>
  </ul>
</div>
...
1. <span class="emphasis">Entering the Area</span>

<div class="centeredBullet">
  <ul class="diamondBullets fullWidthBullet">
    <li>Your laptop's DHCP client contacts the local DHCP server via a wireless connection.</li>
  </ul>
</div>

2. <span class="emphasis">Receiving an Address</span>

<div class="centeredBullet">
  <ul class="diamondBullets fullWidthBullet">
    <li>The DHCP server assigns an IPv4 address to your laptop.</li>
  </ul>
</div>
...
<div class="centeredBullet">
  <ul class="diamondBullets fullWidthBullet">
    <li>With home networks, the DHCP server may be located at the ISP, with a host on the home network receiving its IPv4 configuration directly from the ISP.</li>
    <li>Various types of devices can be DHCP servers, as long as they are running DHCP service software.</li>
    <li>With most medium to large networks, the DHCP server is usually a local dedicated PC-based server.</li>
  </ul>
</div>
...
1. <span class="emphasis">Acting as a Client</span>

<div class="centeredBullet">
  <ul class="diamondBullets fullWidthBullet">
    <li>The wireless router acts as a client to receive its IPv4 configuration from the ISP.</li>
    <li>It receives the public IPv4 address from the ISP.</li>
  </ul>
</div>

2. <span class="emphasis">Acting as a Server</span>

<div class="centeredBullet">
  <ul class="diamondBullets fullWidthBullet">
    <li>The router then acts as a DHCP server for internal hosts on the local network.</li>
    <li>In this role, it distributes private addresses to internal hosts.</li>
  </ul>
</div>
...
<div class="centeredBullet">
  <ul class="diamondBullets fullWidthBullet">
    <li>Source IP: 0.0.0.0 (the client has no IP yet).</li>
    <li>Destination IP: 255.255.255.255 (the limited broadcast address, which reaches all devices on the local subnet).</li>
    <li>It includes the client's MAC address (in the CHADDR field) and is sent over UDP ports 67/68 (RFC 2131).</li>
    <li>It is a <span class="emphasis">limited broadcast</span> (reaches all devices on the local subnet; routers do not forward it unless DHCP relay is configured).</li>
    <li>The client may set the <span class="emphasis">broadcast flag</span> (the leftmost bit in the 16-bit <span class="emphasis">flags</span> field) to 1 to request that the server reply via broadcast instead of unicast (useful if the client's IP stack cannot yet accept unicast packets before full configuration; per RFC 2131 §4.1).</li>
  </ul>
</div>
...
This is commonly summarized as the <span class="emphasis">DORA process</span> (Discover, Offer, Request, Acknowledge).
...
<div class="centeredBullet">
  <ul class="diamondBullets fullWidthBullet">
    <li>Defining a range of addresses to hand out</li>
    <li>Setting lease duration</li>
...
1. <span class="emphasis">Lease Assigned</span>

<div class="centeredBullet">
  <ul class="diamondBullets fullWidthBullet">
    <li>IP addresses assigned by DHCP are typically leased for a period of time.</li>
  </ul>
</div>

2. <span class="emphasis">Renewal</span>

<div class="centeredBullet">
  <ul class="diamondBullets fullWidthBullet">
    <li>Before the lease expires, the client attempts to renew the lease to continue using the same IP address.</li>
  </ul>
</div>

3. <span class="emphasis">Expiration</span>

<div class="centeredBullet">
  <ul class="diamondBullets fullWidthBullet">
    <li>If the lease is not renewed, the address may return to the available pool.</li>
  </ul>
</div>
...
<div class="xrefBox">
  <span class="emphasis">See:</span><br />
  <a href="/foundations/basics/devices-and-models/routers/roles-and-types" target="_blank" rel="noopener noreferrer">Foundations → Devices & Models → Routers → Roles & Types</a>
</div>
